...
def estimate_capacity_receiver(pcap: list, adr_skip) -> float:
    """
    Capacity in MBit/s
    ...
    return get_estimate_receiver(ip_pdu_sizes, iats, adr_skip) / 10 ** 6

def get_estimate_receiver(mss: list, iats, adr_skip=False) -> float:
    ...
    capacity = find_capacity(mss, iats, adr_skip=adr_skip)
    return capacity
...
def open_pcap(path: str) -> list:
    ...
    try:
        with open(path, 'rb') as cf:
            return list(dpkt.pcap.Reader(cf))
    except:
        return []
...
def find_capacity(sizes, iats, verbose=False, adr_skip=False):
...
def main():
    ...
    cmd.add_argument("path", help="path to pcap")
    cmd.add_argument("--adrskip", help="Decides whether second phase of pprate should be skipped or not.", action='store_true')
    args = cmd.parse_args()

    if args.path == None:
        print("Missing input pcap")
        exit()

    adr_skip = args.adrskip
    print("Skip ADR phase? %s" % adr_skip)
    pcap = open_pcap(args.path)
    print(args.path)
    
    estimate = estimate_capacity_receiver(pcap, adr_skip)
